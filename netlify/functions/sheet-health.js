  ...
  try {
    const response = await fetch(webhookUrl, {
      ...
      body: JSON.stringify({
        ...
        spreadsheetId: CASH_COUNT_SPREADSHEET_ID,
        branch: "Alphaland",
        rows: [],
        secret,
      }),
    ...
    return {
      statusCode: 200,
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify({ ok: result && result.ok === true, webhook_status: response.status, error: result && result.ok === true ? undefined : String((result && result.error) || "Webhook rejected request") }),
    };
  } catch (error) {
